    ...
    <nav className="bg-[#124170] text-[#DDF4E7] px-6 py-4 shadow-md fixed top-0 left-0 right-0 z-50">
      <div className="max-w-6xl mx-auto flex justify-between items-center">
        ...
        <ul className="hidden md:flex space-x-6 text-sm font-medium">
          ...
          <li><a href={`${import.meta.env.BASE_URL}Sabarna_Resume.pdf`} target="_blank" rel="noopener noreferrer" className="hover:text-[#67C090] transition">Resume</a></li>
        </ul>
      </div>
      ...
      {isOpen && (
        <ul className="md:hidden mt-4 space-y-2 text-sm font-medium">
          ...
          <li><a href={`${import.meta.env.BASE_URL}Sabarna_Resume.pdf`} target="_blank" rel="noopener noreferrer" className="block px-4 py-2 hover:bg-[#26667F] rounded">Resume</a></li>
        </ul>
      )}
    </nav>
  );
